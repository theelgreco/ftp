<template>
  <section class="section w-full h-full flex flex-col justify-center select-none">
    <div class="content md:w-[90%] w-[90%] md:p-16 max-h-[90%] p-14 mx-auto border-1 border-gray-200 shadow-2xl
      ...
      <div class="flex flex-wrap gap-6 w-full h-full overflow-y-auto" v-if="files.length">
        <div v-for="file in files" :key="file.name"
             class="flex flex-col items-center text-center w-[23%] max-w-[100px] p-3 max-h-[100px]"
             :title="file.name"
  ...
  methods: {
    getPathString(){
      ...
      if (this.path.length > 1) {
        URL = URL + `${this.path.slice(1).join("/")}/`
      }
    ...
    async handleFileClick(file) {
      if (file.type === 2) {
        await this.goForward(file.name)
      }
    },
